...
class LibcSearcher(object):
    ...
    def decided(self):
        ...
        if len(result) > 1:
            ...
            while True:
                if self.chooseflag:
                    in_id=self.mychoose
                    print("Your already choose: ",in_id)
                else:
                    in_id = input(
    ...
    def choose(self, num):
        ...
        return
    def showinfo(self):
        self.libc_dir=self.libc_database_path+self.db[:-8]+'.so'
        print("[+] libc dir: " + self.libc_dir)
        print("[+] system offset: ",hex(self.dump("system")))
        print("[+] /bin/sh offset: ",hex(self.dump("str_bin_sh")))
        print('[+] one_gadget')
        ret = subprocess.call(("one_gadget "+self.libc_dir),shell=True)
        # os.system("one_gadget "+self.libc_dir)
        print("-"*20)
        print(ret)
